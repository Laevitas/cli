...
if ! curl -fsSL "$SUMS_URL" -o "$TMP/checksums.txt"; then
  err "Failed to download checksums.txt from $SUMS_URL — refusing to install without integrity verification."
fi
...
if [ -z "$EXPECTED" ]; then
  err "Checksum line for $ARCHIVE not found in checksums.txt — refusing to install."
fi
...
if command -v sha256sum >/dev/null 2>&1; then
  ...
  err "No SHA-256 tool available (need sha256sum, shasum, or openssl) — refusing to install."
fi
